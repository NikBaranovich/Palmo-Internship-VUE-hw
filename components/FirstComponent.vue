  ...
  watch: {
    ...
    user: {
      handler(newValue, oldValue) {
        console.log(`User object has been changed`);
      },
      /**
    ...
    "user.age"(newAge, previousAge) {
      console.log(`Age value changed from: ${previousAge} to: ${newAge}`);
    },
    /**
     * Lesson 2 Task 13: Use a watcher to send API requests when reactive data changes.
     */
    "user.name"(newName, previousName) {
      console.log(`Name value changed from: ${newName} to: ${previousName}`);
    ...
    pressedButton(newValue, previousValue) {
      console.log(`pressedButton value changed from: '${newValue}'' to: '${previousValue}'`);
    },
  },
};
</script>
